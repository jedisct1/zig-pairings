//! Quadratic extension field Fp2 = Fp[u] / (u^2 + 1) for BLS12-381.
//!
...
pub const Fp2 = struct {
    ...
    pub fn fromBytes(bytes: [encoded_length]u8, endian: std.builtin.Endian) NonCanonicalError!Fp2 {
        if (endian == .big) {
            return .{
                .c1 = try Fp.fromBytes(bytes[0..48].*, .big),
                .c0 = try Fp.fromBytes(bytes[48..96].*, .big),
            };
        } else {
            return .{
                .c0 = try Fp.fromBytes(bytes[0..48].*, .little),
                .c1 = try Fp.fromBytes(bytes[48..96].*, .little),
            };
        }
    }
    ...
    pub fn toBytes(fe: Fp2, endian: std.builtin.Endian) [encoded_length]u8 {
        ...
        if (endian == .big) {
            bytes[0..48].* = fe.c1.toBytes(.big);
            bytes[48..96].* = fe.c0.toBytes(.big);
        } else {
            bytes[0..48].* = fe.c0.toBytes(.little);
            bytes[48..96].* = fe.c1.toBytes(.little);
        }
        return bytes;
    }
